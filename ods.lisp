	...
	(check-type value-type (member :float :percentage :currency :date :time :boolean :string)))
      (attr (*ns-office* "value-type") (if value-type
					   ...
					   (typecase (or raw-value content)
					     (real "float")
					     (t "string"))))
      (attr (*ns-office* (case (or value-type (typecase (or raw-value content)
						(real :float)
						(t :string)))
	    ...
	    (cond ((realp raw-value)
		  ...
		  (content))))
    (when span-columns
      (check-type span-columns (integer 1 *))
      (attr (*ns-table* "number-columns-spanned") (princ-to-string span-columns)))
    (when span-rows
      (check-type span-rows (integer 1 *))
      (attr (*ns-table* "number-rows-spanned") (princ-to-string span-rows)))
    (when content
      (with-tag ((*ns-text* "p"))
	(if link
	    (with-tag ((*ns-text* "a"))
